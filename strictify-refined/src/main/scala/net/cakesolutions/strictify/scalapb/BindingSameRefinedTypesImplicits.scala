...
trait StrictifyRefinedTypesImplicits extends StrictifyInstanceHelpers {
  implicit def stricitifyToRefined[T, P](implicit v: Validate[T, P]) =
    instance[T Refined P, T](RefType.applyRef[T Refined P](_).left.map(StrictifyError.apply))
}
...
trait BindingSameRefinedTypesImplicits extends StrictifyRefinedTypesImplicits with LoosenRefinedTypesImplicits